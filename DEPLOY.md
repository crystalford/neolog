...
- `inbox.v1.key_create` (create inbox items via `/api/v1/inbox/[key]`)
...
- `pulse.curate`, `pulse.resolve` (Pulse curation endpoints; session-authenticated)
...
- `posts.distribution_pack`, `posts.expand_section`, `posts.summary`, `posts.syndications.list` (post helper endpoints)
...
- `feeds.global`, `publications.list` (feed + publications endpoints)
- `reading_progress.save` (reading progress endpoint)
- `social_cards.render` (social cards render endpoint)
- `webmention.receive`, `webmention.verify` (webmention endpoints)
- `video.brief.create` (video brief endpoint)
- `video.heygen.avatars`, `video.heygen.voices`, `video.heygen.generate`, `video.heygen.status`, `video.heygen.webhook` (HeyGen video endpoints)
- `video.synthesia.create`, `video.synthesia.status`, `video.synthesia.webhook` (Synthesia video endpoints)
