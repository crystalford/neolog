...
import {
  Plus, Search, Filter, Trash2, Archive, PenLine,
  Link as LinkIcon, Image, Quote, Code, FileText, Zap,
  Globe, Rss, ChevronDown, MoreHorizontal
} from 'lucide-react'
...
export default function CapturesPage() {
  ...
  async function loadCaptures() {
    ...
    const { data } = await supabase.from('vault_assets').select('*').eq('user_id', session.user.id).order('created_at', { ascending: false })
    setCaptures((data||[]).map((item:any)=>({ ...item, processed: item.tags && item.tags.length>0 })))
    setLoading(false)
  }
  ...
  function formatRelativeTime(dateStr:string){
    ...
    if(mins<60) return `${mins}m ago`; if(hrs<24) return `${hrs}h ago`; if(days===1) return 'Yesterday'; if(days<7) return `${days}d ago`; return date.toLocaleDateString()
  }
  ...
  const filteredCaptures = captures.filter(c => {
    ...
    if(search){ const q=search.toLowerCase(); const mTitle=(c.title||'').toLowerCase().includes(q); const mContent=(c.content||'').toLowerCase().includes(q); const mTags=c.tags?.some(t=>t.toLowerCase().includes(q)); if(!mTitle&&!mContent&&!mTags) return false }
    return true
  })
  ...
  if(loading) return (
    <main className="px-8 py-10 max-w-5xl mx-auto animate-fade-up min-h-screen">
      <div className="animate-pulse space-y-8"><div className="h-8 w-48 skeleton rounded"/><div className="h-12 skeleton rounded-2xl"/>
        <div className="space-y-4">{[1,2,3,4].map(i=>(<div key={i} className="h-20 skeleton rounded-2xl"/>))}</div>
      </div>
    </main>
  )
  ...
  if(activeTab==='sources') return (
    <main className="px-6 lg:px-8 py-8 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div><h1 className="font-display text-2xl">Captures</h1><p className="text-sm mt-1">Everything you've saved.</p></div>
        <button onClick={()=>setCaptureModalOpen(true)} className="btn btn-primary btn-sm"><Plus size={16}/> Add Capture</button>
      </div>
      <div className="rounded-2xl border p-8 text-center"><Globe size={32} className="mx-auto mb-4"/><h2 className="font-display text-lg mb-2">Configure Sources</h2>
        <p className="text-sm mb-4">Add RSS feeds, social profiles, or monitors.</p>
        <Link href="/sources" className="btn btn-secondary"><Rss size={16}/> Manage Sources</Link>
      </div>
    </main>
  )
  ...
  return (
    <>
      ...
      <main className="px-8 py-10 max-w-5xl mx-auto min-h-screen">
        <div className="flex items-center justify-between mb-10">
          <div><h1 className="font-display text-3xl">Captures</h1><p className="text-base mt-2">Everything you've saved.</p></div>
          <button onClick={()=>setCaptureModalOpen(true)} className="btn btn-primary"><Plus size={18}/> Add Capture</button>
        </div>

        <div className="flex gap-2 p-2 rounded-2xl mb-8 w-fit">
          {[{id:'all',label:'All',count:captures.length},{id:'inbox',label:'Inbox',count:inboxCount},{id:'vault',label:'Vault',count:vaultCount},...(capabilities.showSourcesMonitors?[{id:'sources',label:'Sources',count:null}]:[])].map(tab=>(
            <button key={tab.id} onClick={()=>setActiveTab(tab.id as any)} className={`px-5 py-2 rounded-xl ${activeTab===tab.id? 'bg-white/90':''}`}>
              {tab.label} {tab.count!==null && <span className="ml-2 text-xs">{tab.count}</span>}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-4 mb-8">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search captures..." value={search} onChange={(e)=>setSearch(e.target.value)} className="w-full pl-12 pr-4 py-3 rounded-2xl" />
          </div>
          <button onClick={()=>setShowFilters(!showFilters)} className="btn btn-secondary"> <Filter size={18}/> Filters <ChevronDown size={16}/> </button>
        </div>

        {showFilters && (
          <div className="mb-8 p-5 rounded-2xl border bg-[var(--bg-secondary)]">
            <p className="text-xs uppercase mb-3">Type</p>
            <div className="flex flex-wrap gap-2">
              <button onClick={()=>setTypeFilter(null)} className={`${!typeFilter?'bg-[var(--accent)] text-white':''}`}>All types</button>
              {['link','image','quote','code','text','prompt'].map(t=> (<button key={t} onClick={()=>setTypeFilter(t)} className={`${typeFilter===t?'bg-[var(--accent)] text-white':''}`}>{t}</button>))}
            </div>
          </div>
        )}
        ...
        {selectedIds.length>0 && (
          <div className="mb-6 p-4 rounded-2xl flex items-center justify-between">
            ...
            <div className="flex gap-3"><button onClick={()=>handleDelete(selectedIds)} className="btn btn-secondary text-[var(--error)]">Delete</button><button onClick={()=>setSelectedIds([])} className="btn btn-secondary">Clear</button></div>
          </div>
        )}
        ...
        {filteredCaptures.length>0 ? (
          <div className="space-y-3">
            {filteredCaptures.map((capture,i)=>{
              const Icon = typeIcons[capture.type]||FileText; const isSelected=selectedIds.includes(capture.id)
              return (
                <div key={capture.id} className={`group p-5 rounded-2xl border ${isSelected?'border-[var(--accent)]':''}`}>
                  <div className="flex items-start gap-4">
                    <input type="checkbox" checked={isSelected} onChange={(e)=>{ if(e.target.checked) setSelectedIds([...selectedIds,capture.id]); else setSelectedIds(selectedIds.filter(id=>id!==capture.id))}} />
                    <span className="w-12 h-12 flex items-center justify-center"><Icon size={20}/></span>
                    <div className="flex-1 min-w-0"><p className="truncate">{capture.title||capture.content.slice(0,60)}</p><div className="text-xs mt-2">{capture.type} · {formatRelativeTime(capture.created_at)}</div></div>
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100"><button onClick={()=>handleStartDraft(capture)}><PenLine size={18}/></button><button><MoreHorizontal size={18}/></button></div>
                  </div>
                </div>
              )
            })}
          ...
          <div className="rounded-2xl border border-dashed p-16 text-center">
            <Archive size={36} className="mx-auto mb-4" />
            <h2 className="font-display text-xl mb-2">{search?'No matches found':'Nothing captured yet'}</h2>
            <p className="text-base mb-6">{search?'Try a different search term or clear filters.':'Ideas are everywhere. Start capturing.'}</p>
            {!search && (<button onClick={()=>setCaptureModalOpen(true)} className="btn btn-primary"><Zap size={18}/> Capture your first idea</button>)}
          </div>
        )}
      </main>
    </>
