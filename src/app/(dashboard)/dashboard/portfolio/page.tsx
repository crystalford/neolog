...
import { 
  ...
  Download, Copy, ExternalLink, TrendingUp, Zap
} from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import type { Entity, ProjectDocument } from '@/types/database'

type ProjectWithDoc = Entity & {
...
export default function PortfolioPage() {
  ...
  const [projects, setProjects] = useState<ProjectWithDoc[]>([])
  ...
  const loadData = async () => {
    ...
    const [
      ...
      supabase.from('project_documents').select('*').eq('user_id', session.user.id)
    ])

    const projectsWithDocs = (projectData || []).map(p => ({
      ...p,
      document: documentData?.find(d => d.entity_id === p.id)
    }))
  ...
  const handleGenerateCaseStudy = async (projectId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        // Since this is queued, we'll just show a "Synthesis started" state
        alert('Case study synthesis started. It will appear here in ~30 seconds.')
      }
    } catch (err) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8 md:py-12 space-y-12">
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${
                      hasDoc ? 'bg-blue-400/10 text-blue-400' : 'bg-white/5 text-[var(--text-tertiary)] group-hover:bg-blue-400/10 group-hover:text-blue-400'
                    }`}>
      ...
      {/* Case Study Modal */}
      {activeProject && projects.find(p => p.id === activeProject)?.document && (
                    ...
                    <>
                      <div className="space-y-4">
                        <div className="flex items-center gap-3 text-blue-400">
                          <Briefcase size={20} />
                          <span className="text-xs font-bold uppercase tracking-[0.3em]">Project Living Document</span>
                        </div>
                        <h2 className="text-3xl font-bold">{p.name}</h2>
                        <div className="flex flex-wrap gap-2 pt-2">
                          <span className="px-2 py-1 rounded-lg bg-blue-400/10 text-blue-400 text-[10px] font-mono uppercase">#{p.type}</span>
                          {p.status && <span className="px-2 py-1 rounded-lg bg-emerald-400/10 text-emerald-400 text-[10px] font-mono uppercase">{p.status}</span>}
                        </div>
                      </div>

                      <div className="space-y-4">
                        <h4 className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
                          <BookOpen size={12} className="text-blue-400" /> Origin story
                        </h4>
                        <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{doc.origin_story || 'No origin story captured yet.'}</p>
                      </div>

                      <div className="space-y-4">
                        <h4 className="text-[10px] font-bold text-[var(--text-tertiary)] uppercase tracking-widest flex items-center gap-2">
                           <Target size={12} className="text-red-400" /> Executive summary
                        </h4>
                        <p className="text-sm text-[var(--text-secondary)] leading-relaxed">{doc.overview}</p>
                      </div>

                      {doc.technical_notes && (
                         <div className="p-6 rounded-2xl bg-blue-400/5 border border-blue-400/20 space-y-2">
                            <h4 className="text-[10px] font-bold text-blue-400 uppercase tracking-widest flex items-center gap-2">
                              <Zap size={12} /> Technical Reference
                            </h4>
                            <p className="text-xs text-[var(--text-tertiary)] leading-relaxed whitespace-pre-wrap">{doc.technical_notes}</p>
                         </div>
                      )}

                      <div className="p-6 rounded-2xl bg-emerald-400/5 border border-emerald-400/20 space-y-4">
                        <h4 className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest flex items-center gap-2">
                          <CheckCircle size={12} /> Recent Decisions
                        </h4>
                        <div className="space-y-3">
                          {Array.isArray(doc.decisions_log) && doc.decisions_log.slice(0, 3).map((d: any, i: number) => (
                            <div key={i} className="text-xs text-[var(--text-secondary)]">
                              • <span className="font-bold">{d.decision || d}</span>: {d.reasoning || ''}
                            </div>
                          ))}
                        </div>
                      </div>
