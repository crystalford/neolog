...
/**
 * /p/[id] client-side view. Fetches the public production from
 * /api/p/[id] and renders the editorial body. Split from page.tsx so
 * page.tsx can be a server component and export generateMetadata for
 * OG cards + page title.
 */
    ...
    <PublicShell>
      <article style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          marginBottom: 48, paddingBottom: 18, borderBottom: '1px solid var(--line)',
        }}>
          <span>by <b style={{ color: 'var(--fg-1)', fontFamily: 'inherit' }}>{p.attribution}</b></span>
          <span style={{ color: 'var(--fg-5)' }}>·</span>
          <span>{dateStr}</span>
          {p.engagement?.views != null && (
            <>
              <span style={{ color: 'var(--fg-5)' }}>·</span>
              <span><b style={{ color: 'var(--fg-1)', fontFamily: 'inherit' }}>{p.engagement.views.toLocaleString()}</b> views</span>
            </>
          )}
        </div>
        <div style={{
