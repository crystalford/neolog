...
export function PageViewClient({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/v2/pages/${id}`, { cache: 'no-store' })
  ...
  const loadAsk = useCallback(async () => {
    try {
      ...
      const mine = (j.questions || []).find(
        q => q.target_kind === 'page' && q.target_id === id,
      )
