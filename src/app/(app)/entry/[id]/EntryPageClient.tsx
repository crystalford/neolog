...
export function EntryPageClient({ id }: { id: string }) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetch(`/api/v2/log/${id}`, { cache: 'no-store' })
  ...
  const splitAt = useCallback(async (atWord: number) => {
    const res = await fetch(`/api/v2/log/${id}/split`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
