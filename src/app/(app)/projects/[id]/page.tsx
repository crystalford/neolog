...
/**
 * Project detail — the Pack-Rats-style creative container, distinct
 * from /production/[id] (the real generated-script/video engine output).
 * Renamed from /productions/[id] to /projects/[id] to match the
 * underlying `projects` D1 table and remove the naming collision.
 *
 * Sections:
 *   1. Crumbs (Timeline / Projects / project name)
 *   2. Hero — Project tag pill + 64-84px h1 + logline pull-quote +
 ...
 *
 * Data: /api/v2/projects/[id] (table is `projects`).
 */
...
export default function ProjectDetailPage({ params }: { params: { id: string } }) {
  ...
  if (error) return (
    <Shell>
      <CanonCrumbs trail={[{ label: 'Timeline', href: '/' }, { label: 'Projects', href: '/projects' }, 'Error']}/>
      <div style={{ padding: 40, color: 'var(--t-terra)' }}>Error: {error}</div>
    </Shell>
  )
  if (!project) return (
    <Shell>
      <CanonCrumbs trail={[{ label: 'Timeline', href: '/' }, { label: 'Projects', href: '/projects' }, '…']}/>
    ...
    <Shell>
      <div style={{ ['--topic' as any]: color } as React.CSSProperties}>
        ...
        <section className="canon-prov-grid" style={{ marginTop: 32 }}>
          ...
          <ProvCell label="Project id" value={truncate(project.id, 22)} mono/>
        </section>
