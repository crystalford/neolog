 ...
 *   POST /concat-audio     { input_urls: string[] }       -> one mp3
 *   GET  /health                                          -> "ok"
 *
 * Five, and that is all of them — every one has a caller, and
 * `scripts/check-unreached-routes.mjs` fails CI on a sixth. Seven went on
 * 9 Sep (/trim, /concat, /extract-audio-segment, /extract-video-segment,
 * /render-video-essay, /ken-burns, /images-to-video): the production engine,
 * deleted from the product on 8 Sep and still deployed here, 520 of this
 * file's 1,240 lines.
 *
...
// Stitches N audio inputs (webm/mp4/mp3) into one MP3, in order. The
// pipeline DO calls it to put a recording's audio chunks back together
// before Whisper reads them — a long recording is uploaded in pieces by the
// browser, and Whisper wants one file.
...
const BUILD_VERSION = 'five-endpoints-2026-09-09'
