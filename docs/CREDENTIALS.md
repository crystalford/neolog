...
- The Pages project (`neolog`) is provisioned with the custom domain `neolog.ai` attached — **still the live production target as of this writing**. The 19 Sep OpenNext Cloudflare migration (see `CLAUDE.md`) builds and deploys a Worker (also named `neolog`) as the replacement, verified end to end via `wrangler deploy --dry-run` and a full CI pass, but the domain cutover — pointing `neolog.ai`'s Workers Custom Domain at the new Worker instead of the Pages project — is a deliberately separate, explicit, operator-confirmed step, not yet done. The old Pages project is kept as an instant rollback until the operator says otherwise.
